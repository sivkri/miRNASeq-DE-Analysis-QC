...
#importing required libraries to perform below DE analysia and QC
library("DESeq2")
library("gplots")
library(ggplot2)
library(pheatmap)
...
counts <- read.delim("path_to_your_file/abi_v3.txt", sep="\t", header=T, row.names=1)
...
#to prepare pheatmap (alternative to heatmap)
...
#To prepare sample to sample distance
...
#To prepare Dispersion plot
pdf("Dispersionplot_abi1_interaction_effect.pdf")
plotDispEsts(dds)
dev.off()

#to prepare scatterplot
rld <- rlog(dds, blind=FALSE)
rlogMat <- assay(rld)
...
#to prepare Screeplot(This will give compare individual Principle components)
pdf("Screeplot.pdf")
